...
#define DRIVER_VERSION          "3.5.2 20151221"
...
#ifdef STK_TUNE0
#define STK_MAX_MIN_DIFF	200	//200
#define STK_LT_N_CT			100
#define STK_HT_N_CT		150
#endif

#define STK_TRANSMITTANCE	1100	//FOR Light Sensor
...
static int stk3x1x_enable_ps(struct i2c_client *client, int enable, int validate_reg)
{
	...
	if(enable)
	{
		...
		if(1 == obj->hw->polling_mode_ps)
			wake_lock(&ps_lock);
		//else if(0 == obj->hw->polling_mode_ps)
			//enable_irq(obj->irq);
	}
	else
	{
...
static int stk_ps_tune_zero_init(struct stk3x1x_priv *obj)
{
	...
	obj->ps_high_thd_boot = obj->hw->ps_threshold_high_def;
	obj->ps_low_thd_boot = obj->hw->ps_threshold_low_def;
...
static int stk3x1x_get_als_value(struct stk3x1x_priv *obj, u16 als)
{
	...
	if(!invalid)
	{
		...
		lux_val = (als * 1100) / STK_TRANSMITTANCE;
		return lux_val;	//obj->hw->als_value[idx];
	}
	else
	{
		if(atomic_read(&obj->trace) & STK_TRC_CVT_ALS)
...
#endif
{
	...
	switch (cmd)
	{
		...
		case ALSPS_IOCTL_CLR_CALI: 
			...
			atomic_set(&obj->ps_high_thd_val,  obj->hw->ps_threshold_high_def); 
			atomic_set(&obj->ps_low_thd_val,  obj->hw->ps_threshold_low_def);  //obj->hw->ps_low_thd_val
...
static int als_get_raw_data(int *als_value)
{
	int err = 0;
...
static int ps_get_raw_data(int *ps_value)
{
	int err = 0;
	struct stk3x1x_priv *obj = NULL;
...
static int stk3x1x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	obj->first_boot = true;			 
	obj->als_correct_factor = 1000;
	atomic_set(&obj->ps_high_thd_val, obj->hw->ps_threshold_high_def);//  obj->hw->ps_high_thd_val
	atomic_set(&obj->ps_low_thd_val, obj->hw->ps_threshold_low_def);	
	...
	obj->stk_lt_n_ct = obj->hw->ps_5cm_delta; //STK_LT_N_CT;
	obj->stk_ht_n_ct = obj->hw->ps_3cm_delta; //STK_HT_N_CT;
#endif
	...
	als_data.als_get_raw_data = als_get_raw_data;
	...
	ps_data.ps_get_raw_data = ps_get_raw_data;
